...
def init_user_preferences_db():
    """
    Creates a table to store user-specific GUI preferences.
    Each user can have custom colors, themes, and other GUI settings.
    """
...
def login_user(data):
    ...
    if user:
        ...
        preferences = get_user_preferences(data['username'])
        return {
            "status": "success", 
...
def get_user_preferences(username):
    """
    Retrieves GUI preferences for a user.
    If no preferences exist, creates default preferences.
    Returns a dictionary with all preference values.
    """
    conn = sqlite3.connect('aubus.db')
    c = conn.cursor()
    c.execute('''
        SELECT sidebar_color, background_color, button_color, button_hover_color, 
               text_color, theme_name, font_size
    ...
    if result:
        return {
            "sidebar_color": result[0],
            "background_color": result[1],
            "button_color": result[2],
            "button_hover_color": result[3],
            "text_color": result[4],
            "theme_name": result[5],
            "font_size": result[6]
        }
        ...
        save_user_preferences(username, default_prefs)
        return default_prefs

def save_user_preferences(username, preferences):
    """
    Saves or updates GUI preferences for a user.
    preferences is a dictionary with keys: sidebar_color, background_color, 
    button_color, button_hover_color, text_color, theme_name, font_size
    """
    conn = sqlite3.connect('aubus.db')
    c = conn.cursor()
    c.execute('''
        INSERT OR REPLACE INTO user_preferences 
        (username, sidebar_color, background_color, button_color, button_hover_color, 
         text_color, theme_name, font_size)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    ''', (
        username,
        preferences.get("sidebar_color", "#2c3e50"),
        preferences.get("background_color", "#FFEAEC"),
        preferences.get("button_color", "#2c3e50"),
        preferences.get("button_hover_color", "#34495e"),
        preferences.get("text_color", "white"),
        preferences.get("theme_name", "default"),
        preferences.get("font_size", 14)
    ))
    conn.commit()
    conn.close()
...
if __name__ == "__main__":
    ...
    init_rating_db()
    init_user_preferences_db()  # Initialize user preferences table
